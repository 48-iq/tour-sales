...
RETURNS TABLE(id text, username text, name text, surname text, email text, birth_date date, phone text) AS
$$
BEGIN
    RETURN QUERY
    SELECT u.id, u.username, u.name, u.surname, u.email, u.birth_date, u.phone
    FROM users u
    WHERE u.id = user_id;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION update_user_by_id(p_id text, p_username text, p_name text, p_surname text, p_email text, p_birth_date date, p_phone text)
RETURNS void AS
$$
BEGIN
    UPDATE users
    SET username = p_username, name = p_name, surname = p_surname, email = p_email, birth_date = p_birth_date, phone = p_phone
    WHERE id = p_id;
END;
$$ LANGUAGE plpgsql;
